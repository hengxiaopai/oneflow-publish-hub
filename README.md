...
当前阶段为 **Phase 4，前后端分离 SaaS MVP 基础**。Vanilla JS 工作台保持不变，
新增 Fastify API、Prisma、SQLite、本地 dev session、服务端套餐校验、凭据加密字段
与后端 Mock Publisher Worker。项目不包含真实平台 API Token、Cookie、账号凭证
或支付密钥。
...
选择“本地演示模式”后进入 `localStorage` 工作台。SaaS 云端入口只显示认证占位，
不收集账号或密码，也不会调用假登录接口。
...
后端默认运行在 `http://127.0.0.1:4174`。返回登录页选择 “SaaS Dev Mode”，前端会
调用 `/api/dev/session` 并从后端读取用量、文章、渠道和发布记录。后端未启动时会
提示“后端服务未启动，可切换到本地开发模式。”

默认 SQLite 文件位于 `server/dev.db`，并被 `.gitignore` 排除。完整说明见
